...
import { version, transformSync as transformSync$1, transform as transform$1 } from 'esbuild';
...
console.log({
  ...
  geteuid: typeof process.geteuid === "function" ? process.geteuid() : os.userInfo().username
});
console.log({
  userInfo: os.userInfo().username,
...
function transformSync(code, filePath, extendOptions) {
  ...
  const hash = sha1(code + JSON.stringify(esbuildOptions) + version);
